...
Workflow Comparison:
- OLD (clustering-first): align → cluster → generate consensus → assign via identity
- NEW (haplotype-first): align → mask gaps → extract core → identify ESVs → direct mapping

The new workflow eliminates re-assignment via identity calculation, which would create
false ties when ESV haplotypes differ by 1-2 bp (0.2-0.4% divergence).
